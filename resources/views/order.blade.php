                    ...
                    <div class="card-body">
                        <form action="/order" method="post" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input disabled class="form-control" id="user_id" name="user_id" value="{{{ Auth::user()->name}}}">
                            </div>
                            <div class="form-group">
                                ...
                                <select class="form-control" id="film_id" name="film_id">
                                    ...
                                    @foreach ($ddetail as $item)
                                        <option value="{{$item->id}}">{{$item->judul}} {{ ' | ' }} Tanggal :{{$item->tanggal_tayang}} {{ ' | ' }} Waktu :{{$item->waktu_tayang}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
